  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (!token || !userData?.id) {
        setError("Você precisa estar logado para publicar um imóvel");
        setLoading(false);
        return;
      }
      
      // Função para extrair sigla do estado do endereço
      const extractStateFromAddress = (address: string): string => {
        ...
        // Primeiro, verifica se já é uma sigla válida (2 caracteres)
        const lastPart = address.split(',').slice(-1)[0]?.trim();
        if (lastPart && lastPart.length === 2 && /^[A-Z]{2}$/i.test(lastPart)) {
          return lastPart.toUpperCase();
        }
        
        // Procura por nomes de estados no endereço
        ...
        // Se não encontrar, retorna SP como padrão
        return "SP";
      };

      // Preparar os dados para envio
      const propertyData = {
        proprietario: userData.id, // Campo obrigatório
        titulo: form.title || "Título não informado",
        descricao: form.description || "Descrição não informada",
        tipo: form.propertyType.toLowerCase(),
        preco: parseFloat(form.price.replace(/\s/g, '').replace(',', '.')) || 0,
        endereco: form.address || null, // Agora pode ser null
        cidade: form.address ? (form.address.split(',').slice(-2)[0]?.trim() || "Cidade não informada") : "Cidade não informada",
        estado: extractStateFromAddress(form.address || ""),
        cep: "00000-000", // Campo obrigatório - valor padrão
        quartos: Number(form.rooms) || 1,
        banheiros: Number(form.bathrooms) || 1,
        area: 50.0, // Campo obrigatório - valor padrão
        mobiliado: form.amenities.includes("Mobiliado"),
        aceita_pets: form.tags.includes("Pet Friendly"),
        internet: form.amenities.includes("Wi-fi"),
        estacionamento: form.amenities.includes("Estacionamento"),
      };
      ...
      if (photos.length > 0) {
        ...
        for (let i = 0; i < photos.length; i++) {
          ...
          formData.append('imagens', photos[i]);
          if (i === 0) {
            formData.append('principal', '0'); // Primeira foto como principal
          }
      ...
      let errorMessage = "Não foi possível publicar seu imóvel. Por favor, tente novamente.";
      ...
      if (error.response?.data) {
        if (typeof error.response.data === 'string') {
          ...
          // Se há erros de campo específicos
        ...
        <div className="bg-white p-8 rounded-3xl shadow-md border">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <main className="lg:col-span-2 space-y-6">
              ...
              {step === 2 && (
                <StepLocation form={form} setForm={setForm} onNext={next} onBack={back} />
              )}
              {step === 3 && (
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                >
                  {step < 4 ? "Próximo" : loading ? "Publicando..." : "Publicar"}
                </Button>
              </div>
            </main>
          </div>
